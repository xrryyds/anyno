...
class DistilTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.max_total_length = args.max_total_length
        self.teacher_max_prompt_length = (
            args.teacher_max_prompt_length
            if args.teacher_max_prompt_length is not None
        ...
        if self.use_vllm:
            ...
            if self.vllm_mode == "server":
                ...
                if self.max_total_length is not None:
                    max_model_len = self.max_total_length
    ...
    def _generate_and_score_completions(
        ...
        if self.max_total_length is not None:
            trimmed_completion_ids_list = []
            trimmed_sampling_per_token_logps_list = [] if sampling_per_token_logps_list is not None else None
            for idx, prompt_ids in enumerate(prompt_ids_list):
                remaining = max(1, self.max_total_length - len(prompt_ids))
                trimmed_completion_ids_list.append(completion_ids_list[idx][:remaining])
                if trimmed_sampling_per_token_logps_list is not None:
                    trimmed_sampling_per_token_logps_list.append(sampling_per_token_logps_list[idx][:remaining])
            completion_ids_list = trimmed_completion_ids_list
            sampling_per_token_logps_list = trimmed_sampling_per_token_logps_list
