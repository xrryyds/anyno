...
class SDFTWrapperConfig:
    ...
    max_completion_length: int = DEFAULT_MAX_PROMPT_LENGTH
...
def _validate_runtime_paths(config: SDFTWrapperConfig) -> None:
    ...
    if not config.model_path or not str(config.model_path).strip():
        raise FileNotFoundError("Model path is empty. Pass a local model directory or a Hugging Face model id.")
    if os.path.exists(config.model_path) and not os.path.isdir(config.model_path):
        raise FileNotFoundError(f"Model path exists but is not a directory: {config.model_path}")
...
def _infer_lora_target_modules(model) -> List[str]:
    ...
    for group in candidate_groups:
        resolved = [name for name in group if name in module_names]
        if resolved:
            return resolved

    linear_like_suffixes = []
    for name, module in model.named_modules():
        short_name = name.split(".")[-1]
        if short_name in {"lm_head", "embed_tokens", "wte", "word_embeddings"}:
            continue
        class_name = module.__class__.__name__.lower()
        if "linear" in class_name and short_name not in linear_like_suffixes:
            linear_like_suffixes.append(short_name)

    if linear_like_suffixes:
        return linear_like_suffixes[:8]

    raise ValueError(
        "Could not infer LoRA target modules from the model. "
        "Please update `_infer_lora_target_modules` for this architecture."
    )
...
def run_sdft_training_baseline(
    ...
    teacher_max_seq_length: Optional[int] = None,
    ref_model_mixup_alpha: float = 0.01,
    ref_model_sync_steps: int = 1,
    alpha: float = 0.0,
    num_loss_tokens_to_skip: int = 3,
    ...
    effective_max_completion_length = max_completion_length or max(256, max_seq_length - max_prompt_length)
    if max_prompt_length >= max_seq_length:
        logger.warning(
            "max_prompt_length >= max_seq_length. Shrinking prompt budget to keep room for completions."
        )
        max_prompt_length = max(256, max_seq_length // 2)
        effective_max_completion_length = max(256, max_seq_length - max_prompt_length)

    effective_teacher_max_length = teacher_max_seq_length or min(
        DEFAULT_TEACHER_MAX_PROMPT_LENGTH,
        max(max_prompt_length * 2, max_prompt_length + 512),
    )
    wrapper_config = SDFTWrapperConfig(
        ...
        teacher_max_prompt_length=effective_teacher_max_length,
    ...
    logger.info(f"SDFT max_prompt_length: {wrapper_config.max_prompt_length}")
    logger.info(f"SDFT max_seq_length(student total): {max_seq_length}")
    logger.info(f"SDFT max_completion_length: {wrapper_config.max_completion_length}")
    logger.info(f"SDFT teacher_max_seq_length: {wrapper_config.teacher_max_prompt_length}")
    ...
    meta = {
        "train_type": "sdft_original_wrapper_compat",
        ...
        "num_train_epochs": real_data_epochs,
        "num_prompts_per_batch": effective_num_prompts,
        "max_prompt_length": wrapper_config.max_prompt_length,
        "max_seq_length": max_seq_length,
        "max_completion_length": wrapper_config.max_completion_length,
        "teacher_max_seq_length": wrapper_config.teacher_max_prompt_length,
        "ref_model_mixup_alpha": ref_model_mixup_alpha,
        "ref_model_sync_steps": ref_model_sync_steps,
        "alpha": alpha,
        "num_loss_tokens_to_skip": num_loss_tokens_to_skip,
...
def _default_data_path() -> str:
    return os.path.join(_project_root(), "datasets", "exam", "adv_hints.json")


def _candidate_model_paths() -> List[str]:
    root = _project_root()
    return [
        os.environ.get("SDFT_MODEL_PATH", ""),
        os.environ.get("MODEL_PATH", ""),
        os.path.join(root, "model", "OREAL", "OREAL-7B"),
        os.path.join(root, "model", "Qwen", "Qwen2.5-Math-7B-Instruct"),
        os.path.join(root, "model", "Qwen", "Qwen2.5-7B-Instruct"),
        os.path.join(root, "models", "OREAL", "OREAL-7B"),
        os.path.join(root, "models", "Qwen", "Qwen2.5-Math-7B-Instruct"),
        os.path.join(root, "models", "Qwen", "Qwen2.5-7B-Instruct"),
    ]
...
def _resolve_default_model_path() -> str:
    for candidate in _candidate_model_paths():
        if candidate and os.path.isdir(candidate):
            return candidate
    return os.environ.get("SDFT_MODEL_PATH") or os.environ.get("MODEL_PATH") or ""


def _parse_args():
    parser = argparse.ArgumentParser(description="SDFT baseline compatibility wrapper for CELPO")
    parser.add_argument("--model_path", type=str, default=None, help="Local model dir or HF model id.")
    parser.add_argument("--data_path", type=str, default=_default_data_path())
    parser.add_argument("--output_base_dir", type=str, default=os.path.join(_project_root(), "output"))
    parser.add_argument("--batch_size", type=int, default=4)
    parser.add_argument("--real_data_epochs", type=int, default=2)
    parser.add_argument("--device_num", type=int, default=1)
    parser.add_argument("--lora_path", type=str, default=None)
    parser.add_argument("--learning_rate", type=float, default=5e-5)
    parser.add_argument("--num_prompts_per_batch", type=int, default=None)
    parser.add_argument("--max_prompt_length", type=int, default=DEFAULT_MAX_PROMPT_LENGTH)
    parser.add_argument("--max_seq_length", type=int, default=DEFAULT_MAX_TOTAL_LENGTH)
    parser.add_argument("--max_completion_length", type=int, default=None)
    parser.add_argument("--teacher_max_seq_length", type=int, default=None)
    parser.add_argument("--ref_model_mixup_alpha", type=float, default=0.01)
    parser.add_argument("--ref_model_sync_steps", type=int, default=1)
...
if __name__ == "__main__":
    args = _parse_args()
    model_path = args.model_path or _resolve_default_model_path()
    if not model_path:
        raise SystemExit(
            "No model path found. Pass --model_path or set SDFT_MODEL_PATH to a local model directory."
        )
    run_sdft_training_baseline(
        model_path=model_path,
        data_path=args.data_path,
        batch_size=args.batch_size,
        real_data_epochs=args.real_data_epochs,
        ...
        teacher_max_seq_length=args.teacher_max_seq_length,
