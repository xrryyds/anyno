...
# ==========================================
# 4. SIRA Trainer (支持 DDP)
# ==========================================

class SiraTrainer(Trainer):
    def __init__(self, hint_config: HintSFTConfig, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.hint_config = hint_config
        self.running_gen_loss = self.hint_config.beta 

    # [DDP 关键修改] 移除了 get_train_dataloader 的重写
    # 让 Trainer 自动使用 DistributedSampler 来处理多卡数据切分
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 在 DDP 模式下，update 仍然在每张卡上运行，但只有主进程会写日志
        if self.model.training:
            tracker.update(loss.item(), metadata, debug_info_list, current_alpha)
    ...
    def adaptive_gating_loss(self, logits, labels, hint_masks, answer_masks):
        ...
        # alpha = max(alpha.item(), 0.1) # 可选：Alpha 下限
...
class StepLogCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        # [DDP 修改] 仅 Rank 0 记录日志
        if args.process_index == 0 and state.global_step > 0 and state.global_step % self.log_interval == 0:
            stats = tracker.get_window_stats() 
            stats["epoch"] = state.epoch
            stats["global_step"] = state.global_step
            stats["timestamp"] = datetime.now().isoformat()
...
def main():
    ...
    # 2. Dataset
    dataset = Dataset.from_json(hint_config.data_path)
    # [DDP 强烈建议] 必须 Shuffle，因为移除了 SequentialSampler
    dataset = dataset.shuffle(seed=42) 
    if local_rank == 0 or local_rank == -1:
        logger.info(f"Loaded dataset from {hint_config.data_path}, size: {len(dataset)}")

    # 3. Model
    # [DDP 修改] 正确设置 device_map
    device_map = {"": local_rank} if local_rank != -1 else "auto"

    model = AutoModelForCausalLM.from_pretrained(
        hint_config.model_path, 
        torch_dtype=torch.float16, 
        device_map=device_map, # <--- 适配 DDP
        trust_remote_code=True
    )
    model.config.use_cache = False  
    ...
    if local_rank == 0 or local_rank == -1:
        logger.info(f"Starting SIRA training (DDP Compatible) with dynamic Alpha...")
