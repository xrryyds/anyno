...
class HintSFTConfig:
    ...
    data_path: str = "./datasets/exam/train_irdcl.json" # 预处理好的数据路径
...
class TrainingMetricsTracker:
    ...
    def update(self, loss, metadata, debug_info):
        ...
        # 解析 batch 内的统计 (Metadata 和 debug_info 是列表)
        # 注意：Trainer 传入的 metadata 是整个 batch 的
        for meta, info in zip(metadata, debug_info):
            mode = meta.get("mode")
            # debug_info 格式: {"gate": float, "loss_contrib": float}
...
class FixedModeCollator:
    ...
    def __call__(self, batch):
        ...
        for item in batch:
            q = item['question']
            b = item.get('hints', "")
            c = item.get('answer') # 这里的 answer 可能是 ref_answer 也可能是 student_answer，视预处理而定
            data_type = item.get('type', 'anchor_data') # 默认当作 anchor 防止报错
            ...
            if data_type == 'anchor_data':
                ...
                # Mask 定义
                # Prompt ... [Hint Start ... Hint End] ... [Answer Start ... End]
...
class SequentialTrainer(Trainer):
    ...
    def get_train_dataloader(self) -> DataLoader:
        """
        重写 DataLoader 获取逻辑。
        强制使用 SequentialSampler，以保证预处理数据中的 (Anchor+Hint) 组合顺序不被打乱。
        """
        ...
        return DataLoader(
            ...
            sampler=SequentialSampler(train_dataset), # 关键：使用顺序采样
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        loss, debug_info_list = self.adaptive_gating_loss(logits, labels, hint_masks, answer_masks)
    ...
    def adaptive_gating_loss(self, logits, labels, hint_masks, answer_masks):
        ...
        for i in range(token_losses.size(0)):
            ...
            if h_count > 0:
                ...
                debug_info_list.append({"gate": 0.0, "loss_contrib": avg_a_loss.item()}) # Anchor Gate 记为 0

        return torch.stack(batch_losses).mean(), debug_info_list
...
class EpochLogCallback(TrainerCallback):
    """
    在每个 Epoch 结束时，计算统计平均值并写入文件。
    """
    def __init__(self, log_file):
        self.log_file = log_file

    def on_epoch_begin(self, args, state, control, **kwargs):
        tracker.reset() # 新 Epoch 开始，重置计数器
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        print(f"\n[Epoch {state.epoch:.2f} Stats] Loss: {stats['avg_train_loss']:.4f} | "
              ...
              f"AnchorLoss: {stats['avg_anchor_loss']:.4f} | "
...
def main():
    ...
    # 2. 加载预处理好的 Dataset
    # 必须保证这个 json 文件是按照 chunk 生成好的 (Hint + Anchor 混合)
    ...
    # 5. 训练参数
    # 注意：设置 dataloader_drop_last=True 以避免最后一个 batch 比例失调
    training_args = TrainingArguments(
        output_dir=output_dir,
        num_train_epochs=10,             # 可配置 Epoch 数
        per_device_train_batch_size=2,   # 根据显存调整，需配合梯度累积
        gradient_accumulation_steps=4,   # 确保 total batch size 对应数据预处理时的 batch size
        learning_rate=5e-5,
        warmup_ratio=0.1,
        logging_steps=10,
        fp16=True,
        ...
        dataloader_drop_last=True,       # 丢弃最后不足 batch_size 的数据，保证梯度构成稳定
        group_by_length=False,           # 禁止按长度排序，必须按顺序读取！
        report_to="none"                 # 禁用 wandb 等，手动记录 log
    )
