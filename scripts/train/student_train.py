...
# 如果导入失败，使用默认格式（注意：GEN_PROMPT 在这里主要用于 Hint 的内容拼接）
try:
...
class TrainingMetricsTracker:
    ...
    def reset_window(self):
        ...
        self.win_anchor_losses_raw, self.win_mode_b_losses = [], []
        self.current_alpha = 0.0 
        self.win_counts = {"anchor": 0, "mode_b": 0}
    def reset_epoch(self):
        self.ep_loss, self.ep_steps = 0.0, 0
        self.ep_gate_values, self.ep_anchor_losses_weighted = [], []
        self.ep_anchor_losses_raw, self.ep_mode_b_losses = [], []
        self.ep_counts = {"anchor": 0, "mode_b": 0}
    def update(self, loss, metadata, debug_info, current_alpha):
        self.current_alpha = current_alpha
        ...
        for meta, info in zip(metadata, debug_info):
            if info is None: continue 
            mode, gate_val, loss_contrib = meta.get("mode"), info.get("gate", 0.0), info["loss_contrib"]
            if mode == "mode_b_generation":
    ...
    def _calculate_stats(self, loss_sum, steps, gate_vals, anchor_w, anchor_raw, mode_b, counts, alpha):
        avg = lambda l: sum(l)/len(l) if l else 0.0
        return {
            "avg_train_loss": loss_sum / max(steps, 1), "avg_gate_value": avg(gate_vals),
            "avg_anchor_loss_weighted": avg(anchor_w), "avg_anchor_loss_raw": avg(anchor_raw),    
            "avg_mode_b_loss": avg(mode_b), "final_alpha": alpha, "sample_counts": counts
        }
    def get_window_stats(self):
        return self._calculate_stats(self.win_loss, self.win_steps, self.win_gate_values, self.win_anchor_losses_weighted, self.win_anchor_losses_raw, self.win_mode_b_losses, self.win_counts, self.current_alpha)
    def get_epoch_stats(self):
        return self._calculate_stats(self.ep_loss, self.ep_steps, self.ep_gate_values, self.ep_anchor_losses_weighted, self.ep_anchor_losses_raw, self.ep_mode_b_losses, self.ep_counts, self.current_alpha)

tracker = TrainingMetricsTracker()

# ==========================================
# 3. Data Collator (关键修改点！)
# ==========================================

class FixedModeCollator:
    def __init__(self, tokenizer, max_length: int = 2048): # 建议增加 max_length 适应 chat 格式
        self.tokenizer = tokenizer
        self.max_length = max_length
        if self.tokenizer.pad_token_id is None:
             self.tokenizer.pad_token_id = self.tokenizer.eos_token_id
    ...
    def __call__(self, batch):
        ...
        for item in batch:
            ...
            # ------------------------------------------------------------------
            # 统一使用 apply_chat_template 构造 Prompt
            # ------------------------------------------------------------------
            
            # 1. 构造 Prompt 部分 (User Input + Chat Template)
            # 这会生成类似: "<|im_start|>user\n{q}<|im_end|>\n<|im_start|>assistant\n"
            ...
            # Tokenize Prompt
            # 注意：apply_chat_template 已经包含了特殊 token，所以 add_special_tokens=False
            prompt_ids = self.tokenizer(prompt_str, add_special_tokens=False).input_ids
            len_prompt = len(prompt_ids)

            # 2. 构造 Output 部分 (Answer / Hint+Answer)
            ...
            if data_type == 'anchor_data':
                ...
                # 设置 Answer Mask (Prompt 之后全是 Answer)
                ...
                # 这里的逻辑是：模型需要学会输出 Hint，然后输出 Answer
                # 我们假设 GEN_HINTS_WIH_ANSWER 格式为: "# known:\n{hints}\n{answer}"
                # 你可以根据实际情况调整这个模板，确保它和 data_math.py 里的一致
                
                # 构造包含 Hint 和 Answer 的文本
                ...
                # 确定 Hint 部分的长度
                # 假设 target_text 的前一部分是 Hint
                # 构造仅包含 Hint 的文本片段来计算长度
                hint_only_text = f"# known:\n{b}\n" # 这是一个假设的分割点，请根据 GEN_HINTS_WIH_ANSWER 调整
                ...
                # 设置 Hint Mask
                hint_end_idx = len_prompt + len_hint_part
                
                # 边界保护：防止 Hint 长度计算超出总长度
                hint_end_idx = min(hint_end_idx, len(full_ids))
            ...
            # 4. 生成 Labels (-100 表示忽略 Loss)
            # 我们只计算 Hint (h_mask) 和 Answer (a_mask) 部分的 Loss
            # Prompt 部分不计算 Loss
            labels = [full_ids[i] if (h_mask[i] or a_mask[i]) else -100 for i in range(len(full_ids))]
...
class StepLogCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        if state.global_step > 0 and state.global_step % self.log_interval == 0:
            ...
            logger.info(f"[Step {state.global_step}] Loss: {stats['avg_train_loss']:.4f} | RawAnchor: {stats['avg_anchor_loss_raw']:.4f} | ModeB: {stats['avg_mode_b_loss']:.4f}")
            tracker.reset_window()
...
class EpochLogCallback(TrainerCallback):
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        stats.update({"epoch": state.epoch, "global_step": state.global_step, "timestamp": datetime.now().isoformat()})
        with open(self.log_file, "a", encoding="utf-8") as f: f.write(json.dumps(stats) + "\n")
        logger.info(f"="*60); logger.info(f"*** EPOCH {state.epoch} FINISHED ***")
        logger.info(f"  Avg Epoch Loss: {stats['avg_train_loss']:.4f}"); logger.info(f"  Avg Gate:       {stats['avg_gate_value']:.4f}")
        logger.info(f"  Avg ModeB Loss: {stats['avg_mode_b_loss']:.4f}"); logger.info(f"  Avg Raw Anchor: {stats['avg_anchor_loss_raw']:.4f}")
        logger.info(f"="*60)
        tracker.reset_epoch()
